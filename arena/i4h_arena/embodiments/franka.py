...
from i4h_arena.tensor_utils import quat_from_euler_degrees as _euler_to_quats  # noqa: E402
...
class FrankaUltrasoundEmbodiment(_EmbodimentBase):
    ...
    def __init__(self, enable_cameras: bool = True, initial_pose=None):
        ...
        if not self.enable_cameras:
            self.observation_config.policy.wrist = None
...
class FrankaUltrasoundRLEmbodiment(FrankaUltrasoundEmbodiment):
    """Probe embodiment with flat, camera-free observations for online RL."""

    name: str = "franka_ultrasound_rl"

    def __init__(self):
        super().__init__(enable_cameras=False)
        # Images and the phantom location belong to the visual scan policy, not
        # this low-dimensional reach objective.  The task contributes probe and
        # target poses in its own ``task_obs`` group.
        self.observation_config.policy.object_position = None
        self.observation_config.policy.concatenate_terms = True
        self.scene_config.robot.init_state.joint_pos = {
            f"panda_joint{index}": value for index, value in enumerate(FRANKA_ULTRASOUND_READY_JOINT_POS, start=1)
        }
        # Bound one normalized action to a small Cartesian delta.  This keeps
        # early PPO exploration from commanding metre-scale IK jumps.
        self.action_config.arm_action.scale = 0.05
    ...
    ee_to_us_transform: _FrameTransformerCfg = _FrameTransformerCfg(
        ...
        target_frames=[
            _FrameTransformerCfg.FrameCfg(
                ...
                offset=_OffsetCfg(
                    pos=[0.0, 0.0, 0.0],
                    rot=_euler_to_quats(_torch.tensor([0.0, 0.0, -90.0])),
                ),
            )
        ],
    )
...
class _CameraSceneCfg:
    ...
    room_camera: _CameraCfg = _CameraCfg(
        ...
        offset=_CameraCfg.OffsetCfg(
            ...
            rot=_euler_to_quats(_torch.tensor([248.0, 0.0, 180.0])),
            convention="ros",
        ),
        colorize_semantic_segmentation=True,
        semantic_segmentation_mapping=_FRANKA_SEMANTIC_MAPPING,
...
class _ActionsCfg:
    arm_action: _IKActionCfg = _IKActionCfg(
        ...
        body_offset=_IKActionCfg.OffsetCfg(
            pos=[0.0, 0.0, 0.0],
            rot=_euler_to_quats(_torch.tensor([0.0, 0.0, 0.0])),
        ),
    )


from i4h_arena.envcfg.panda_phantom import object_position_in_robot_root_frame as _object_position  # noqa: E402

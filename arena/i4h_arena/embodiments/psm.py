...
from i4h_arena.assets.constants import DVRK_PSM_USD
...
def _physical_joint_pos(home_joint_pos: list[float] | tuple[float, ...] | None) -> dict[str, float]:
    ...
    if len(values) == len(_ARM_NAMES) + len(_GRIPPER_NAMES):
        return dict(zip((*_ARM_NAMES, *_GRIPPER_NAMES), values, strict=True))
    ...
    return dict(
        zip(
            (*_ARM_NAMES, *_GRIPPER_NAMES),
            (*arm, -aperture, aperture),
            strict=True,
        )
    )
...
PSM_ROOM_CAMERA_CFG = TiledCameraCfg(
    ...
    offset=TiledCameraCfg.OffsetCfg(
        pos=(0.22, -0.32, 0.28),
        rot=(0.83348, 0.239039, -0.137336, -0.478864),
        convention="ros",
    ),
    data_types=["rgb"],

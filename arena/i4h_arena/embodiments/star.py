...
"""STAR Arena embodiment carried forward from the legacy robotic-surgery runtime."""
...
from i4h_arena.assets.constants import STAR_USD
...
STAR_CFG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        ...
        rigid_props=PhysxRigidBodyPropertiesCfg(disable_gravity=False, max_depenetration_velocity=5.0),
    ...
    init_state=ArticulationCfg.InitialStateCfg(
        joint_pos=dict(zip((*_ARM_NAMES, *_TOOL_NAMES), _DEFAULT_HOME, strict=True)),
    ),
    actuators={
...
STAR_ROOM_CAMERA_CFG = TiledCameraCfg(
    ...
    offset=TiledCameraCfg.OffsetCfg(
        pos=(0.62, -0.36, 0.62),
        rot=(0.791743, 0.372507, -0.276788, -0.395891),
        convention="ros",
    ),
    data_types=["rgb"],
...
class StarEmbodiment:
    ...
    def get_scene_cfg(self) -> Any:
        self.scene_config.robot.init_state.joint_pos = dict(
            zip((*_ARM_NAMES, *_TOOL_NAMES), self.home_joint_pos_rad, strict=True)
        )

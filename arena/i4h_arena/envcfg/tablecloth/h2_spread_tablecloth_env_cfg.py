...
from i4h_arena.assets._tablecloth import CLOTH_INNER_USD, CLOTH_ONLY_USD, TABLE_USD, CameraPresets
...
from i4h_arena.envcfg.tablecloth.cloth_physics import make_newton_physics
from i4h_arena.envcfg.tablecloth.mdp.observations import get_robot_joint_states
...
class H2SpreadTableclothSceneCfg(InteractiveSceneCfg):
    ...
    front_camera = CameraPresets.h2_front_camera(
        focal_length=10.5,
        # Keep the optical center ahead of the head/chest geometry.
        pos_offset=(0.12, 0.0, 0.0),
        # ROS optical frame, XYZW: aim 45 degrees down toward the tabletop.
        rot_offset=(0.65328148, -0.65328148, 0.27059805, -0.27059805),
    )

    # Ground plane at z=0, supporting the robot's feet.
    ground = AssetBaseCfg(
...
class H2ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        robot_joint_state = ObsTerm(func=get_robot_joint_states)

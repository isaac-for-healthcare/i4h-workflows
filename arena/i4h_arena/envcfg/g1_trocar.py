...
1. Joint mappings (shared with ``i4h_arena.embodiments.g1``).
2. Observations.
3. Reset events.
4. Task-stage tracking + reward shaping.
5. Termination predicates.
6. Configclasses + the :class:`G1TrocarEnvCfg` recipe consumed by the env.
"""
...
def get_assemble_trocar_joint_positions(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    return to_torch(robot.data.joint_pos)[:, joint_ids]
...
def reset_tray_with_random_rotation(
    ...
    tray_state = to_torch(tray.data.default_root_state)[env_ids].clone()
    trocar_1_state = to_torch(trocar_1.data.default_root_state)[env_ids].clone()
    trocar_2_state = to_torch(trocar_2.data.default_root_state)[env_ids].clone()
    ...
    delta_quat[:, 2] = torch.sin(half_angle)
    delta_quat[:, 3] = torch.cos(half_angle)
    ...
    tray.write_root_pose_to_sim_index(root_pose=tray_state[:, :7], env_ids=env_ids)
    trocar_1.write_root_pose_to_sim_index(root_pose=trocar_1_state[:, :7], env_ids=env_ids)
    trocar_2.write_root_pose_to_sim_index(root_pose=trocar_2_state[:, :7], env_ids=env_ids)
...
def update_task_stage(
    ...
    pos1 = to_torch(obj1.data.root_pos_w)
    pos2 = to_torch(obj2.data.root_pos_w)
    quat1 = to_torch(obj1.data.root_quat_w)
    quat2 = to_torch(obj2.data.root_quat_w)
...
def object_drop_termination(
    ...
    dropped = (to_torch(env.scene[asset_cfg1.name].data.root_pos_w)[:, 2] < drop_height_threshold) | (
        to_torch(env.scene[asset_cfg2.name].data.root_pos_w)[:, 2] < drop_height_threshold
    )
...
def _robot_hands_near_open(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    current = to_torch(robot.data.joint_pos)[:, joint_ids]
    target = to_torch(robot.data.default_joint_pos)[:, joint_ids]
...
def _store_assemble_trocar_state(
    ...
    obj1: RigidObject = env.scene[asset_cfg1.name]
    obj2: RigidObject = env.scene[asset_cfg2.name]
    pos1 = to_torch(obj1.data.root_pos_w).detach().clone()
    pos2 = to_torch(obj2.data.root_pos_w).detach().clone()
    tip1 = _trocar_tip_position(env, asset_cfg1)
    tip2 = _trocar_tip_position(env, asset_cfg2)
    stage = get_task_stage(env).detach().clone()
    angle = _trocar_axis_angle(to_torch(obj1.data.root_quat_w), to_torch(obj2.data.root_quat_w))
...
def _trocar_tip_position(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    return to_torch(obj.data.root_pos_w) + quat_apply(to_torch(obj.data.root_quat_w), local_tip.repeat(env.num_envs, 1))

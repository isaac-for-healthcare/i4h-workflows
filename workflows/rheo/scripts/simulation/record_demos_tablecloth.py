...
# [DEBUG] Dump native + Python stacks on a fatal signal (SIGSEGV/SIGABRT) so the
# CUDA-700 crash leaves a stack trace instead of a silent exit. Also force
# synchronous CUDA so the error surfaces at the *real* offending kernel/launch
# (must be set before any CUDA context is created, i.e. before Kit starts).
faulthandler.enable(all_threads=True)
if os.environ.get("DEBUG_CUDA_BLOCKING", "1").lower() in ("1", "true", "yes"):
...
parser.add_argument(
    ...
    help="Physics backend preset: 'newton' (coupled MJWarp+VBD, zero-gravity cloth setup) or 'physx'.",
)
...
# [FIX] wp.config.verify_cuda MUST stay off in this script. cloth_physics.py now
# runs with use_cuda_graph=True, and Newton DEFERS the first CUDA-graph capture
# until the first env.step() (RTX mode). verify_cuda forces a cudaDeviceSynchronize
# after every wp.launch -- which is illegal during graph capture. In test_spread
# the manager catches it ("wp.capture_begin failed: Cannot use CUDA error
# verification during graph capture") and falls back to eager. In record_demos the
# first env.step() happens *only* after B is pressed (the pre-B loop is render-only),
# and the path under teleop does not fall back cleanly -- the illegal sync escalates
# to CUDA-700 (Warp CUDA error 700: an illegal memory access was encountered) inside
# narrow_phase/create_soft_contacts on the very first step, i.e. the crash you see on B.
# Default is now OFF. Opt-in only when use_cuda_graph is False (e.g. tablecloth task
# with cloth_physics.use_cuda_graph=False) AND you really need per-launch checking.
...
def _create_env_cfg(output_dir: str, output_name: str):
    ...
    # Resolve the switchable physics backend preset (Newton by default) onto
    # env_cfg.sim.physics. Must run before gym.make.
...
def run(env, success_term, rate_limiter, use_isaac_teleop: bool) -> int:
    ...
    def handle_reset():
        ...
        # Do NOT reset the SimulationContext here by default. With Newton deformables
        # and use_cuda_graph=True, calling env.sim.reset() after the environment has
        # already been initialized rebuilds Newton/CUDA state and re-captures the graph.
        # A clean no-XR reproduction in cloth_interaction_demo.py showed that this
        # exact sequence:
        #
        #   render/wait -> env.sim.reset() -> env.reset() -> first env.step()
        #
        # can poison CUDA state and surface as cudaErrorIllegalAddress / misaligned
        # address on the first post-B step. env.reset() is enough for episode reset.
        # Keep the old behavior opt-in for comparison only.
        if os.environ.get("RECORD_DO_SIM_RESET", "0").lower() in ("1", "true", "yes"):
            env.sim.reset()
        env.reset()
        teleop.reset()

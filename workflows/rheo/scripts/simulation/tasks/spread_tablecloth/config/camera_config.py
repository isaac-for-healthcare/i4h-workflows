...
class CameraPresets:
    ...
    def h2_front_camera(cls, **overrides) -> CameraCfg:
        ...
        NOTE: the H2 USD nests all links under an extra ``H2`` xform (the
        articulation root is ``Robot/H2/pelvis``), so the camera must mount on
        ``Robot/H2/head_yaw_link`` - NOT ``Robot/head_yaw_link``. Under Newton,
        the camera FrameView walks up for the parent physics body; with the
        wrong (un-nested) path it finds none and aborts with
        "FrameView ... needs Newton cloned-world transforms".
        """
        params = {"prim_path": "/World/envs/env_.*/Robot/H2/head_yaw_link/RobotHeadCam"}

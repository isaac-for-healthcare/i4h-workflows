...
# NOTE on z: the H2_sharpa USD already lifts the robot via an authored
# ``H2`` Xform ``translate=(0, 0, 1.05)`` (pelvis local is 0), i.e. the asset
# itself stands the robot at 1.05 m. Adding another 1.05 here double-counts the
# height and the (base-fixed) robot ends up floating ~1.1 m above the floor.
# Keep z ~0 so the asset's built-in lift alone places the feet on the ground
# (ground plane sits at _TABLE_TOP_Z - 2*_TABLE_HALF_HEIGHT = 0.02 m). x/y are
# applied only once (the H2 Xform offset is z-only), so they stay unchanged.
H2_SPREAD_TABLECLOTH_INIT_POS: Tuple[float, float, float] = (-0.95, 0.0, 0.0)

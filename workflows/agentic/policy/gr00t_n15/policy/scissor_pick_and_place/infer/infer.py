...
def add_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--data-config", type=str, default=SO_ARM_POLICY_CONFIG.data_config)
    p.add_argument("--embodiment-tag", type=str, default=SO_ARM_POLICY_CONFIG.embodiment_tag)
    p.add_argument("--denoising-steps", type=int, default=SO_ARM_POLICY_CONFIG.denoising_steps)
...
def run(args: argparse.Namespace) -> None:
    ...
    runner = GR00TPolicyRunner(
        RunnerConfig(
            ...
            action_head_future_tokens=SO_ARM_POLICY_CONFIG.action_head_future_tokens,
...
def _validate_args(args: argparse.Namespace) -> None:
    if args.denoising_steps < 1:
        raise SystemExit("--denoising-steps must be >= 1")

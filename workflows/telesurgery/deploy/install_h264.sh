...
echo "Installing gxf multimedia libraries and corresponding dependencies"
...
for extension in "${!gxf_mm_extensions[@]}"; do
  ...
  curl -L ${extension_url} -o ${extension_tar}
...
rm -rf gxf-mm
ls -l ${HOLOSCAN_LIBS_DIR}libgxf_video*.so
...
if [[ $ARCH == "x86_64" ]]; then
  curl -L ${DS_DEPS_URL} -o ds_deps.deb
  dpkg -i ./ds_deps.deb
  rm ds_deps.deb
fi

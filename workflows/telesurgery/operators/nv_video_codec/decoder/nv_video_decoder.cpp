...
namespace holoscan::ops
{
                                 ...
                                 ExecutionContext &context)
  {
    ...
    do
    {
      ...
      if (nFrameReturned > 1)
      {
        HOLOSCAN_LOG_WARN("More than one frame returned from decoder");
      }
    ...
    if (nFrame > 0)
    {
      ...
      std::map<std::string, uint64_t> stats = {
          {"video_decoder_enter_timestamp", enter_timestamp},
          {"video_decoder_emit_timestamp", emit_timestamp},
          {"jitter_time", emit_timestamp - last_emit_timestamp_},
          {"fps", last_emit_timestamp_ == 0 ? 0 : static_cast<uint64_t>(1e9 / (emit_timestamp - last_emit_timestamp_))}};

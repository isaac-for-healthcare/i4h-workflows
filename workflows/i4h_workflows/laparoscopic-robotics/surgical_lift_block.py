...
"""dVRK PSM lifts a peg-transfer block with its rule-based controller."""
...
def success(ctx) -> object:
    ...
    # The root is at world z=0.15 m, so this is world z > 0.075 m.
    return object_above(ctx.scene, "block", -0.075)
...
WORKFLOW = Workflow(
    ...
    success=success,
    modes={
        "rule-based": lambda: cartesian_lift("block", success=success),
        "replay": replay,
        "idle": idle,
    },
)

...
#
# Vendored/adapted from the Isaac Lab Newton rod solver stack for
# catheter_navigation integration.
...
Usage::
    ...
    from fluorosim.catheter import XPBDRodSolver, RodConfig
...
@wp.func
def _xr_load_blk(blks: wp.array(dtype=wp.float32), blk: int) -> tuple[wp.mat33, wp.mat33, wp.mat33, wp.mat33]:
    base = blk * 36
    A = wp.mat33(
        blks[base + 0],
        blks[base + 1],
...
@wp.func
def _xr_store_blk(
    blks: wp.array(dtype=wp.float32),
    blk: int,
    A: wp.mat33,
    B: wp.mat33,
    C: wp.mat33,
...
@wp.func
def _xr_blk_col(blks: wp.array(dtype=wp.float32), blk: int, col: int) -> tuple[wp.vec3, wp.vec3]:
    base = blk * 36
...
def _xr_compute_jacobians(
    ...
    jrot[_xr_jidx(i, 0, 0)] = 0.0
    jrot[_xr_jidx(i, 0, 1)] = r0.z
    jrot[_xr_jidx(i, 0, 2)] = -r0.y
    jrot[_xr_jidx(i, 1, 0)] = -r0.z
    jrot[_xr_jidx(i, 1, 1)] = 0.0
    jrot[_xr_jidx(i, 1, 2)] = r0.x
    jrot[_xr_jidx(i, 2, 0)] = r0.y
    jrot[_xr_jidx(i, 2, 1)] = -r0.x
    jrot[_xr_jidx(i, 2, 2)] = 0.0

    jrot[_xr_jidx(i, 0, 3)] = 0.0
    jrot[_xr_jidx(i, 0, 4)] = -r1.z
    jrot[_xr_jidx(i, 0, 5)] = r1.y
    jrot[_xr_jidx(i, 1, 3)] = r1.z
    jrot[_xr_jidx(i, 1, 4)] = 0.0
    jrot[_xr_jidx(i, 1, 5)] = -r1.x
    jrot[_xr_jidx(i, 2, 3)] = -r1.y
    jrot[_xr_jidx(i, 2, 4)] = r1.x
    jrot[_xr_jidx(i, 2, 5)] = 0.0
    ...
    if i > 0:
        ...
        for row in range(6):
            for col in range(6):
                ...
                jp_vec = wp.vec3(
                    jrot[_xr_jidx(prev, row, 3)],
                    jrot[_xr_jidx(prev, row, 4)],
                    jrot[_xr_jidx(prev, row, 5)],
                )
                jc_vec = wp.vec3(
                    jrot[_xr_jidx(i, col, 0)],
                    jrot[_xr_jidx(i, col, 1)],
                    jrot[_xr_jidx(i, col, 2)],
                )
                val += wp.dot(jp_vec, _xr_invI_v(inv_inertia, i, jc_vec))
...
def _xr_assemble_darboux_blocks(
    ...
    for row in range(3):
        for col in range(3):
            val = float(0.0)
            jr0r = wp.vec3(
                jrot[_xr_jidx(i, row + 3, 0)],
                jrot[_xr_jidx(i, row + 3, 1)],
                jrot[_xr_jidx(i, row + 3, 2)],
            )
            jr0c = wp.vec3(
                jrot[_xr_jidx(i, col + 3, 0)],
                jrot[_xr_jidx(i, col + 3, 1)],
                jrot[_xr_jidx(i, col + 3, 2)],
            )
            val += wp.dot(jr0r, _xr_invI_v(inv_inertia, i, jr0c))
            jr1r = wp.vec3(
                jrot[_xr_jidx(i, row + 3, 3)],
                jrot[_xr_jidx(i, row + 3, 4)],
                jrot[_xr_jidx(i, row + 3, 5)],
            )
            jr1c = wp.vec3(
                jrot[_xr_jidx(i, col + 3, 3)],
                jrot[_xr_jidx(i, col + 3, 4)],
                jrot[_xr_jidx(i, col + 3, 5)],
            )
            val += wp.dot(jr1r, _xr_invI_v(inv_inertia, i + 1, jr1c))
            if row == col:
    ...
    for row in range(3):
        for col in range(3):
            val = float(0.0)
            jp_vec = wp.vec3(
                jrot[_xr_jidx(prev, row + 3, 3)],
                jrot[_xr_jidx(prev, row + 3, 4)],
                jrot[_xr_jidx(prev, row + 3, 5)],
            )
            jc_vec = wp.vec3(
                jrot[_xr_jidx(i, col + 3, 0)],
                jrot[_xr_jidx(i, col + 3, 1)],
...
class _Workspace:
    ...
    def __init__(self, num_points: int, num_edges: int, device: str):
        ...
        self.constraint_values = wp.zeros(n_dofs, dtype=wp.float32, device=d)
        self.compliance = wp.zeros(n_dofs, dtype=wp.float32, device=d)
        self.lambda_sum = wp.zeros(n_dofs, dtype=wp.float32, device=d)
        self.jacobian_pos = wp.zeros(ne * 36, dtype=wp.float32, device=d)
        self.jacobian_rot = wp.zeros(ne * 36, dtype=wp.float32, device=d)

        self.rhs = wp.zeros(n_dofs, dtype=wp.float32, device=d)
        self.delta_lambda = wp.zeros(n_dofs, dtype=wp.float32, device=d)
...
class XPBDRodSolver:
    ...
    def __init__(
        ...
        *,
        ...
        if solver_backend not in XPBD_DIRECT_SOLVE_BACKENDS:
            raise ValueError(f"solver_backend must be one of {XPBD_DIRECT_SOLVE_BACKENDS!r}, got {solver_backend!r}")
    ...
    def _substep(self, dt: float) -> None:
        ...
        wp.launch(
            ...
            inputs=[
                ws.orientations,
                ws.angular_velocities,
                ws.torques,
                ws.quat_inv_masses,
                dt,
                self._angular_damping,
                ws.predicted_orientations,
            ],
            device=dev,
        )
        ...
        wp.launch(_xr_zero_f, dim=n_dofs, inputs=[ws.lambda_sum], device=dev)
        wp.launch(
            _xr_prepare_compliance,
            dim=ne,
            inputs=[
                ws.rest_lengths,
                ws.bend_stiffness,
    ...
    def _pre_constraints_hook(self, ws: "_Workspace", dt: float, dev: str) -> None:
        ...
        Override in a subclass (e.g. :class:`~fluorosim.catheter.XCathRodSolver`)
    ...
    def _project_direct(self, ws: _Workspace, dt: float, dev: str) -> None:
        ...
        wp.launch(
            ...
            inputs=[
                ...
                ws.rest_lengths,
                ws.rest_darboux,
                ws.constraint_values,
            ],
            device=dev,
        )

        wp.launch(
            _xr_compute_jacobians,
            dim=ne,
            inputs=[
                ws.predicted_orientations,
                ws.rest_lengths,
    ...
    def _solve_system(self, ws: _Workspace, dt: float, dev: str) -> "wp.array":
        ...
        if self._solver_backend == XPBD_DIRECT_SOLVE_BANDED_CHOLESKY:
            if ws._ab is None:
                ws._ab = wp.zeros((_XPBD_BAND_LDAB, max(1, n_dofs)), dtype=wp.float32, device=dev)
            wp.launch(
                _xr_zero_2d,
                dim=_XPBD_BAND_LDAB * max(1, n_dofs),
                inputs=[
                    ws._ab,
                    int(_XPBD_BAND_LDAB),
                    int(max(1, n_dofs)),
            ...
            wp.launch(_xr_spbsv_u11_1rhs, dim=1, inputs=[int(n_dofs), ws._ab, ws.rhs], device=dev)
            return ws.rhs

        # Default: block_thomas (with tiled Cholesky for small systems)
        if n_dofs <= _XPBD_TILE:
            if ws._A is None:
    ...
    def _solve_split_thomas(self, ws: _Workspace, ne: int, dev: str) -> "wp.array":
        ...
        wp.launch(
            ...
            inputs=[
                ...
                ws.inv_inertia,
                int(ne),
                ws._split_stretch_diag,
                ws._split_stretch_offdiag,
            ],
            device=dev,
        )
        wp.launch(
            _xr_assemble_darboux_blocks,
            dim=ne,
            inputs=[
                ws.jacobian_rot,
                ws.compliance,
                ws.inv_inertia,
                int(ne),
                ws._split_darboux_diag,
                ws._split_darboux_offdiag,
            ],
            device=dev,
        )

        wp.launch(
            _xr_build_rhs_stretch,
            dim=ne,
            inputs=[
                ws.constraint_values,
                ws.compliance,
                ws.lambda_sum,
                int(ne),
                ws._split_stretch_rhs,
            ],
            device=dev,
        )
        wp.launch(
            _xr_build_rhs_darboux,
            dim=ne,
            inputs=[
                ws.constraint_values,
                ws.compliance,
                ws.lambda_sum,
                int(ne),
                ws._split_darboux_rhs,
            ],
            device=dev,
        )

        wp.launch(
            _xr_block_thomas_3x3,
            dim=1,
            inputs=[
                ws._split_stretch_diag,
                ws._split_stretch_offdiag,
                ...
                int(ne),
                ws._split_stretch_c,
                ws._split_stretch_d_prime,
                ws._split_stretch_dl,
            ],
            device=dev,
        )
        wp.launch(
            ...
            inputs=[
                ...
                int(ne),
                ws._split_darboux_c,
                ws._split_darboux_d_prime,
                ws._split_darboux_dl,
            ],
            device=dev,
        )

        wp.launch(
            _xr_merge_delta_lambda,
            dim=ne,
            inputs=[
                ws._split_stretch_dl,
                ws._split_darboux_dl,
    ...
    def _substep_batched(self, dt: float) -> None:
        ...
        dev = self._device
        tp = bws.np_total
        total_edges = bws.ne_total
        td = bws.nd_total

        # 1. Predict positions & rotations (per-env gravity)
        wp.launch(
            _xr_predict_pos_batched,
            dim=tp,
            inputs=[
                bws.positions,
                bws.velocities,
        ...
        wp.launch(
            _xr_prepare_compliance_batched,
            dim=total_edges,
            inputs=[
                bws.rest_lengths,
                bws.bend_stiffness,
    ...
    def _project_direct_batched(self, bws: _BatchedWorkspace, dt: float, dev: str) -> None:
        ...
        wp.launch(
            _xr_update_constraints_batched,
            dim=total_edges,
            inputs=[
                bws.predicted_positions,
                bws.predicted_orientations,
        ...
        wp.launch(
            _xr_compute_inv_inertia_batched,
            dim=tp,
            inputs=[
                bws.predicted_orientations,
                bws.quat_inv_masses,
        ...
        wp.launch(
            _xr_compute_corrections_batched,
            dim=total_edges,
            inputs=[
                bws.inv_masses,
                bws.quat_inv_masses,
    ...
    def apply_proximal_control(
        ...
        if abs(rotate_velocity) > 1e-10:
            ...
            dq = torch.tensor(
                ...
                device=ori_torch.device,
                dtype=ori_torch.dtype,
            )

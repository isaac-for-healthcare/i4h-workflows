...
class RodData:
    ...
    def _init_material(self):
        ...
        # Segment lengths
        self.segment_lengths = torch.full((n, s), geo.segment_length, device=self.device, dtype=torch.float32)
    ...
    def _init_constraints(self):
        ...
        self.bend_twist_compliance = torch.zeros((n, s - 1, 3), device=self.device, dtype=torch.float32)
    ...
    def _create_warp_arrays(self):
        ...
        self.wp_angular_velocities = wp.from_torch(self.angular_velocities.view(-1, 3), dtype=wp.vec3f)
        self.wp_prev_positions = wp.from_torch(self.prev_positions.view(-1, 3), dtype=wp.vec3f)
        self.wp_prev_orientations = wp.from_torch(self.prev_orientations.view(-1, 4), dtype=wp.quatf)
        ...
        self.wp_lambda_bend_twist = wp.from_torch(self.lambda_bend_twist.view(-1, 3), dtype=wp.vec3f)
        self.wp_lambda_shear = wp.from_torch(self.lambda_shear.view(-1, 2), dtype=wp.vec2f)
        self.wp_stretch_compliance = wp.from_torch(self.stretch_compliance.view(-1), dtype=wp.float32)
        self.wp_shear_compliance = wp.from_torch(self.shear_compliance.view(-1, 2), dtype=wp.vec2f)
        self.wp_bend_twist_compliance = wp.from_torch(self.bend_twist_compliance.view(-1, 3), dtype=wp.vec3f)
    ...
    def get_endpoint_positions(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        local_offset = torch.tensor([[-seg_len / 2, 0, 0]], device=self.device).expand(self.num_envs, -1)

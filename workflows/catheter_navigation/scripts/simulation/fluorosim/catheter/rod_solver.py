...
from .rod_kernels import solve_stretch_bend_fused_kernel  # Performance: fused constraint kernel
...
class DirectTreeSolver:
    ...
    def _build_system(self, rod_data: RodData, dt: float):
        ...
        for i in range(c):
            ...
            self.rhs[:, i, :3] = -(
                stretch_residual[:, i] + stretch_compliance * rod_data.lambda_stretch[:, i : i + 1] / dt2
            )
            self.rhs[:, i, 3:] = -(bend_twist_residual[:, i] + bend_compliance * rod_data.lambda_bend_twist[:, i] / dt2)
            ...
            # Add compliance
            self.diag_blocks[:, i, :3, :3] += stretch_compliance.unsqueeze(2) * torch.eye(3, device=self.device) / dt2
    ...
    def _solve_tridiagonal(self):
        ...
        for i in range(c):
            ...
            try:
                ...
                self.delta_lambda[:, i] = torch.matmul(torch.linalg.pinv(A), b.unsqueeze(-1)).squeeze(-1)
...
class RodSolver:
    ...
    ```python
    from fluorosim.catheter import RodSolver, RodConfig
    ...
    def __init__(
        ...
        if self.config.solver.use_direct_solver:
            self.direct_solver = DirectTreeSolver(self.config.geometry.num_segments, num_envs, self.device)
        else:
    ...
    def _update_warp_cache(self):
        ...
        self._cached_wp_inv_inertias_diag = wp.from_torch(inv_inertia_diag.flatten(), dtype=wp.float32)

        # Cache radii (never change)
        if hasattr(self.data, "radii") and self.data.radii is not None:
            self._cached_wp_radii = wp.from_torch(self.data.radii.flatten(), dtype=wp.float32)
    ...
    def _apply_corrections(self, delta_lambda: torch.Tensor, dt: float):
        ...
        # Normalize quaternions
        self.data.orientations = self.data.orientations / (self.data.orientations.norm(dim=-1, keepdim=True) + 1e-8)
    ...
    def _omega_to_quat_batch(omega: torch.Tensor, dt: float = 1.0) -> torch.Tensor:
        ...
        c = torch.where(small_angle.unsqueeze(-1), torch.ones_like(half_angle), torch.cos(half_angle))
    ...
    def _gauss_seidel_iteration(self, dt: float):
        ...
        inv_inertias_diag = wp.from_torch(
            torch.diagonal(self.data.inv_inertias, dim1=-2, dim2=-1).contiguous().view(-1, 3), dtype=wp.vec3f
        )
    ...
    def get_energy(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        inertias_diag = wp.from_torch(
            torch.diagonal(self.data.inertias, dim1=-2, dim2=-1).contiguous().view(-1, 3), dtype=wp.vec3f
        )
        bending_stiffness = wp.from_torch(self.data.bending_stiffness.view(-1, 2), dtype=wp.vec2f)
        torsion_stiffness = wp.from_torch(self.data.torsion_stiffness.view(-1), dtype=wp.float32)

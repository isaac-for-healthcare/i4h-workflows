...
def compute_bending_twist_potential(
    ...
    return (
        0.5
        * length
        * (stiffness[0] * diff[0] * diff[0] + stiffness[1] * diff[1] * diff[1] + stiffness[2] * diff[2] * diff[2])
    )
...
def solve_bend_twist_constraints_kernel(
    ...
    # Δθ = W * J^T * Δλ
    omega_corr = wp.vec3f(J_scale * delta_lambda[0], J_scale * delta_lambda[1], J_scale * delta_lambda[2])

    # Apply corrections to parent (rotate by -correction in local frame)
    if not fixed[parent_idx]:
        corr1 = wp.vec3f(I1_inv[0] * omega_corr[0], I1_inv[1] * omega_corr[1], I1_inv[2] * omega_corr[2])
    ...
    if not fixed[segment_idx]:
        corr2 = wp.vec3f(I2_inv[0] * omega_corr[0], I2_inv[1] * omega_corr[1], I2_inv[2] * omega_corr[2])
...
def compute_total_energy_kernel(
    ...
    ke = 0.5 * m * wp.dot(v, v)
    ke = ke + 0.5 * (
        I_diag[0] * omega[0] * omega[0] + I_diag[1] * omega[1] * omega[1] + I_diag[2] * omega[2] * omega[2]
    )
...
def solve_stretch_bend_fused_kernel(
    ...
    # Constraint error
    C_bend = wp.vec3f(Omega[0] - Omega_rest[0], Omega[1] - Omega_rest[1], Omega[2] - Omega_rest[2])
    ...
    alpha_tilde_bend = wp.vec3f(alpha_bend[0] / (dt * dt), alpha_bend[1] / (dt * dt), alpha_bend[2] / (dt * dt))
    ...
    omega_corr = wp.vec3f(
        J_scale * delta_lambda_bend[0], J_scale * delta_lambda_bend[1], J_scale * delta_lambda_bend[2]
    )

    # Apply to parent (negative direction)
    if not fixed[parent_idx]:
        corr1 = wp.vec3f(w1_rot[0] * omega_corr[0], w1_rot[1] * omega_corr[1], w1_rot[2] * omega_corr[2])
        corr1_world = quat_rotate_vec(q1, corr1)
        dq1 = omega_to_quat_delta(-corr1_world, 1.0)
        q1_new = quat_mul(dq1, q1)
        q1_len = wp.sqrt(q1_new[0] * q1_new[0] + q1_new[1] * q1_new[1] + q1_new[2] * q1_new[2] + q1_new[3] * q1_new[3])
    ...
    if not fixed[segment_idx]:
        corr2 = wp.vec3f(w2_rot[0] * omega_corr[0], w2_rot[1] * omega_corr[1], w2_rot[2] * omega_corr[2])
        corr2_world = quat_rotate_vec(q2, corr2)
        dq2 = omega_to_quat_delta(corr2_world, 1.0)
        q2_new = quat_mul(dq2, q2)
        q2_len = wp.sqrt(q2_new[0] * q2_new[0] + q2_new[1] * q2_new[1] + q2_new[2] * q2_new[2] + q2_new[3] * q2_new[3])

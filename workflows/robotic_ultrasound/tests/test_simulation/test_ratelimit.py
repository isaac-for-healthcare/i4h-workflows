...
class TestRateLimitedCallback(unittest.TestCase):
    ...
    def test_rate_limiting(self):
        ...
        callback = RateLimitedCallback(
            "test", target_period, self.test_callback, self.mock_world, adaptive_period=False
        )
        ...
        delta = 2
        self.assertAlmostEqual(self.callback_called, expected_calls, delta=delta)

    @parameterized.expand(TEST_CASES)
    def test_adaptive_rate_adjustment(self, name, exec_count, real_time, interval_time):
        callback = RateLimitedCallback("test", 1 / 10, self.test_callback, self.mock_world, adaptive_period=True)
    ...
    def test_get_stats(self):
        callback = RateLimitedCallback("test", 1 / 10, self.test_callback, self.mock_world)
...
class TestPhysXCallbacks(unittest.TestCase):
    ...
    def test_remove_physx_callbacks(self):
        ...
        self.assertEqual(self.mock_world.remove_physics_callback.call_count, 2)

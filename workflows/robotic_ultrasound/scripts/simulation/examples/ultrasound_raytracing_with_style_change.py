...
class Simulator(Operator):
    ...
    def setup(self, spec: OperatorSpec):
        ...
        for filename, material in mesh_configs:
            success, world, meshes = add_mesh(filename, material, self.world, self.meshes)
            if success:
                success_count += 1
        ...
        try:
            ...
            self.simulator_liver = rs.RaytracingUltrasoundSimulator(self.world_liver, self.materials)
        except Exception as e:
        ...
        scan_area_liver = self.simulator_liver.generate_scan_area(
            self.probe,
    ...
    def compute(self, op_input, op_output, context):
        ...
        # Run simulation
        b_mode_image_liver = self.simulator_liver.simulate(self.probe, self.sim_params)
        ...
        if contours:
            ...
            grey_data_liver_to_scan_area = self._add_liver_to_scan_area(grey_data_liver, main_contour)
            # add gan
            grey_data_liver_to_scan_area = Image.fromarray(grey_data_liver_to_scan_area)
            grey_data_liver_to_scan_area = self.img_transforms(grey_data_liver_to_scan_area)
            grey_data_liver_to_scan_area = grey_data_liver_to_scan_area.unsqueeze(0).cuda()
            grey_data_liver_to_scan_area = generate_us(grey_data_liver_to_scan_area, self.generator)
            
            op_output.emit({"": grey_data_liver_to_scan_area}, "output")
        else:

...
class BaseAnnotator:
    ...
    Attributes:
        name: Annotator identifier
        prim_path: USD path to the sensor primitive
        stage: Current USD stage context
        sensor_prim: Reference to the sensor primitive in the stage
        publishers: Active DDS publishers
        subscribers: Active DDS subscribers
    """
    ...
    def start(self, world) -> None:
        ...
        for publisher in self.publishers:
            add_physx_step_callback(f"{publisher.topic}", publisher.hz, publisher.write, world)
        for subscriber in self.subscribers:
            add_physx_step_callback(f"{subscriber.topic}", subscriber.hz, subscriber.read, world)
            subscriber.start()

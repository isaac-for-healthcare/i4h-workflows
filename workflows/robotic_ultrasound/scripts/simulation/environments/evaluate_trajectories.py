...
from simulation.evaluation.metrics import (
    compute_trajectory_overlap_and_distance
)
    ...
    for e in range(episode):
        ...
        if gt_actions is None or gt_states is None:
            ...
            for method_name_skip in PREDICTION_SOURCES.keys():
                ...
                for r_val_skip in radius_to_test:
                    results_per_method[method_name_skip]["all_episodes_success_rates_per_radius"][r_val_skip].append(np.nan)
            continue
        ...
        for method_name, details in PREDICTION_SOURCES.items():
            ...
            if pred_traj is None or pred_traj.shape[0] == 0:
                print(f"  {method_name} - Ep {e}: Predicted trajectory is empty or failed to load. Skipping metrics for this prediction.")

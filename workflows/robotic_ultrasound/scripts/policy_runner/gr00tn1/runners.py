...
class GR00TN1PolicyRunner:
    ...
    def infer_g1(self, room_img, wrist_img, current_state) -> torch.Tensor:
        # Prepare input data with batch dimension for model
        data_point = {
            "video.rs_view": np.expand_dims(room_img, axis=0),
            "video.rm_view": np.expand_dims(wrist_img, axis=0),
            "state.body": np.expand_dims(np.array(current_state[0:29]), axis=0),
            "state.hands": np.expand_dims(np.array(current_state[29:43]), axis=0),
            "annotation.human.task_description": [self.task_description],
        }

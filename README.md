...
This repository currently includes following workflows:
...
- **[Agentic](./workflows/agentic/README.md)** - End-to-end Physical AI workflow for building agentic healthcare robotics tasks with IsaacLab-Arena, GR00T, and openpi policies. Provides configurable environments for SO-ARM 101, Unitree G1, and Franka-style robots across manipulation, loco-manipulation, trocar assembly, and ultrasound scanning scenarios. Includes teleoperation recording, HDF5 replay, trajectory mimicry, VLM-based success annotation, LeRobot dataset conversion, optional Cosmos Transfer augmentation, policy fine-tuning, and closed-loop validation tools.  This workflow is designed to be driven by Claude Code using the skills under `.claude/skills/i4h-workflow*`.
- **[Catheter Navigation](./workflows/catheter_navigation/README.md)** - GPU-accelerated endovascular catheter and guidewire navigation simulator pairing a physics-accurate fluoroscopy (DRR) renderer with an XPBD Cosserat-rod solver and a patient-specific vasculature digital twin, forming a data factory for imitation- and reinforcement-learning policies on tasks such as TAVR guidance, coronary intervention, and endovascular navigation. Features a Slang differentiable DRR renderer with fused Beer-Lambert catheter compositing and a DSA pipeline, XPBD rod solvers with vessel-mesh containment, CT ingestion (DICOM/NIfTI) with TotalSegmentator vessel meshing and centerline extraction, and an interactive fluoroscopy viewport with multi-projection C-arm control.

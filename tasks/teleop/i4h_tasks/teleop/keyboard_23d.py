...
class KeyboardTo23DAdapter:
    ...
    def __init__(self, cfg: KeyboardTo23DConfig):
        self.cfg = cfg
        ...
        self._state = {
            ...
            "left_wrist_quat": np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32),
            "right_wrist_pos": np.array(cfg.default_right_hand_pos, dtype=np.float32),
            "right_wrist_quat": np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32),
    ...
    def _apply_quaternion_delta(self, key: str, delta_rpy: np.ndarray):
        delta_quat_xyzw = self._euler_quaternion(delta_rpy)
        current_quat = self._state[key]
        new_quat = self._quaternion_multiply(current_quat, delta_quat_xyzw)
        norm = np.linalg.norm(new_quat)
        if norm > 1e-6:
            self._state[key] = new_quat / norm
        else:
            self._state[key] = np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32)
    ...
    def _process_hand_input(self, key_name: str, hand: str):
        ...
        if key_name == "W":
            ...
            if norm > 1e-6:
                ...
                self._state[quat_key] = np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32)
    ...
    @staticmethod
    def _euler_quaternion(rpy: np.ndarray) -> np.ndarray:
        """XYZ Euler delta to an ``xyzw`` quaternion without a SciPy dependency."""
        roll, pitch, yaw = (float(value) * 0.5 for value in rpy)

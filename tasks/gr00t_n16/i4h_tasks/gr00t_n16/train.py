...
"""Finetuning entry for the gr00t_n16 stack.

    uv run --project tasks/gr00t_n16 i4h-gr00t-n16 -train \\
        --task gr00t_n16/<name> --dataset <lerobot-dir> [--output-dir ...]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gr00t_n16-train", description="Finetune a gr00t_n16 checkpoint.")
    parser.add_argument("--task", required=True, help="task id, e.g. gr00t_n16/scissor_pick_and_place")
    ...
    parser.add_argument("--modality-config-path", default=None, help="overrides [task.train].modality_config_path")
...
def main(argv: list[str] | None = None) -> int:
    ...
    logging.basicConfig(level=logging.DEBUG if args.verbose else logging.INFO, format="[gr00t_n16-train] %(message)s")
    ...
    if project != "gr00t_n16":
        print(
            f"error: {args.task} is served by {project!r}, not 'gr00t_n16'. Run that project's trainer instead.",
            file=sys.stderr,
        )
        return 2

...
"""GR00T N1.6 fine-tuning body for G1 loco-manipulation tasks."""
...
from i4h_common.training import default_base_model, train_default

logger = logging.getLogger("i4h_tasks.gr00t_n16.train")
_TASK_ID = "gr00t_n16/locomanip_tray_pick_and_place"
...
class TrainConfig:
    ...
    output_dir: str = field(default_factory=lambda: train_default(_TASK_ID, "output_dir", "/tmp/gr00t_g1"))

    base_model_path: str = field(default_factory=lambda: default_base_model(_TASK_ID, "nvidia/GR00T-N1.6-3B"))
    embodiment_tag: str = "NEW_EMBODIMENT"
    modality_config_path: str | None = None
    video_backend: str = "decord"

    batch_size: int = field(default_factory=lambda: train_default(_TASK_ID, "batch_size", 32))
    max_steps: int = field(default_factory=lambda: train_default(_TASK_ID, "max_steps", 20000))
    save_steps: int = field(default_factory=lambda: train_default(_TASK_ID, "save_steps", 2000))
    num_gpus: int = 1

    tune_llm: bool = False
    tune_visual: bool = field(default_factory=lambda: train_default(_TASK_ID, "tune_visual", True))
...
def _groot_root() -> Path:
    return workflow_root() / "third_party" / "Isaac-GR00T-1.6"


def _modality_config_path(cfg: TrainConfig) -> Path:
    if cfg.modality_config_path:
        path = Path(cfg.modality_config_path).expanduser()
        return path.resolve() if path.is_absolute() else (workflow_root() / path).resolve()
    return Path(__file__).resolve().parent / "locomanip" / "config_dualcam.py"
...
def run(cfg: TrainConfig) -> None:
    ...
    # Pin visibility before the first CUDA runtime query. Otherwise a
    # single-GPU request on a multi-GPU host can still expose every device to
    # Transformers and silently activate incompatible DataParallel behavior.

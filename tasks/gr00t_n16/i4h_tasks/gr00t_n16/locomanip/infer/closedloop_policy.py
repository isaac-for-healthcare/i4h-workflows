...
from i4h_tasks.gr00t_n16.locomanip.infer.joint_conversion import remap_policy_joints_to_sim_joints
from i4h_tasks.gr00t_n16.locomanip.infer.tensorrt_dit import replace_dit_with_tensorrt

logger = logging.getLogger("policy")
_WORKFLOW_POLICY_CONFIG_FIELDS = {"pov_cam_names_sim"}
...
class G1LocomanipClosedloopPolicy:
    ...
    def _config_fields(self, yaml_data: dict[str, Any]) -> dict[str, Any]:
        ...
        config_fields.update(_WORKFLOW_POLICY_CONFIG_FIELDS)
    ...
    def _resolve_model_path(self) -> Path:
        ...
        if not self.model_repo:
            raise FileNotFoundError(
                "No G1 model repo configured. Pass a model path override or set model.repo in the task manifest."
            )

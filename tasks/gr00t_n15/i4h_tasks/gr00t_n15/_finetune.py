...
"""GR00T fine-tuning body for the gr00t_n15 stack.

Defaults come from the task manifest, so `[task.model]` is the single place a
checkpoint's hyper-parameters live — the same block the inference server reads.
...
while preserving the released SO-ARM training defaults.
"""
...
from i4h_common.training import default_base_model, task_default, train_default

logger = logging.getLogger("i4h_tasks.gr00t_n15.train")
#: Set by the CLI before TrainConfig is instantiated.
_TASK_ID = "gr00t_n15/scissor_pick_and_place"
...
class TrainConfig:
    ...
    output_dir: str = train_default(_TASK_ID, "output_dir", "/tmp/gr00t_so_arm")
    data_config: str = task_default(_TASK_ID, "data_config", "so100_dualcam")
    action_head_future_tokens: int = task_default(_TASK_ID, "action_head_future_tokens", 0)

    batch_size: int = 32
    max_steps: int = train_default(_TASK_ID, "max_steps", 10000)
    save_steps: int = train_default(_TASK_ID, "save_steps", 1000)
    num_gpus: int = 1

    base_model_path: str = field(default_factory=lambda: default_base_model(_TASK_ID, "nvidia/GR00T-N1.5-3B"))
...
def _train(cfg: TrainConfig) -> None:
    ...
    model = GR00T_N1_5.from_pretrained(
        ...
        action_head_future_tokens=cfg.action_head_future_tokens,
    )
        ...
        def on_log(self, args, state, control, logs=None, **kwargs):
            for name in ("loss", "grad_norm"):
                ...
                if value is not None and not math.isfinite(float(value)):
                    raise FloatingPointError(f"non-finite {name} at training step {state.global_step}: {value}")

    runner = gr00t_runner.TrainRunner(
        train_dataset=train_dataset,
        model=model,
        training_args=training_args,
        resume_from_checkpoint=cfg.resume,
    )
    runner.trainer.add_callback(FiniteTrainingCallback())
    runner.train()
...
def _validate(cfg: TrainConfig) -> None:
    ...
    if cfg.action_head_future_tokens < 0:
        raise SystemExit("--action-head-future-tokens must be >= 0")
...
def _pin_single_gpu() -> None:
    """Prevent Transformers from silently enabling incompatible DataParallel."""
...
def run(cfg: TrainConfig) -> None:
    """Stable entry point for train.py, independent of this module's internals."""
    _validate(cfg)
    if cfg.num_gpus > 1:
        raise SystemExit(_run_torchrun(cfg))
    _pin_single_gpu()
    _train(cfg)

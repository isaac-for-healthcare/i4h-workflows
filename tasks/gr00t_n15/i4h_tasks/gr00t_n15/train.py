...
"""Finetuning entry for the gr00t_n15 stack.

    uv run --project tasks/gr00t_n15 i4h-gr00t-n15 -train \\
        --task gr00t_n15/<name> --dataset <lerobot-dir> [--output-dir ...]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gr00t_n15-train", description="Finetune a gr00t_n15 checkpoint.")
    parser.add_argument("--task", required=True, help="task id, e.g. gr00t_n15/scissor_pick_and_place")
...
def main(argv: list[str] | None = None) -> int:
    ...
    logging.basicConfig(level=logging.DEBUG if args.verbose else logging.INFO, format="[gr00t_n15-train] %(message)s")
    ...
    if project != "gr00t_n15":
        print(
            f"error: {args.task} is served by {project!r}, not 'gr00t_n15'. Run that project's trainer instead.",
            file=sys.stderr,
        )
        return 2

    from i4h_tasks.gr00t_n15 import _finetune

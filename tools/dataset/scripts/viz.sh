...
DATASET_PROJECT="$REPO_ROOT/tools/dataset"
...
start_viz() {
  if [[ ! -d "$DATASET_PROJECT/.venv" ]]; then
    echo "[lerobot-viz] dataset venv missing; run ./setup.sh first" >&2
    exit 1
  fi
...
Stop with:
  $REPO_ROOT/tools/dataset/scripts/viz.sh --stop --port $PORT --state-dir "$STATE_DIR"
EOF
}

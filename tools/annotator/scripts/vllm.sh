...
IMAGE_NAME="${I4H_ANNOTATOR_VLLM_IMAGE:-nvcr.io/nvidia/vllm:26.03.post1-py3}"
CONTAINER_NAME="${I4H_ANNOTATOR_VLLM_CONTAINER:-i4h-workflows-annotator-vllm}"
MODEL="${I4H_ANNOTATOR_VLLM_MODEL:-Qwen/Qwen3-VL-8B-Instruct}"
PORT="${I4H_ANNOTATOR_VLLM_PORT:-8000}"
...
usage() {
    ...
    echo "  I4H_ANNOTATOR_VLLM_IMAGE=${IMAGE_NAME}"
    echo "  I4H_ANNOTATOR_VLLM_MODEL=${MODEL}"
    echo "  I4H_ANNOTATOR_VLLM_PORT=${PORT}"
    exit 1
}
...
if [[ "${MODE}" == "wait" ]]; then
    deadline=$((SECONDS + ${I4H_ANNOTATOR_VLLM_WAIT_TIMEOUT:-300}))
